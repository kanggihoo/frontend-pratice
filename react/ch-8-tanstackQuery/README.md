...
### staleTime vs gcTime (데이터의 생명주기)

| 옵션 | 의미 | 비유 | 기본값 |
|------|------|--------|--------|
| **`staleTime`** | 데이터를 **신선(Fresh)**하다고 판단하는 시간. 이 시간 동안은 재요청하지 않음 | 우유의 **유통기한** | 0 (즉시 stale) |
| **`gcTime`** | 사용되지 않는 캐시를 **메모리**에 보관하는 시간 (Garbage Collection) | 빈 병을 **분리수거**하기 전 보관 기간 | 5분 |

#### 💡 왜 두 값을 구분하고, 왜 달라도 될까요?
- **구분의 이유:** `staleTime`은 "언제 서버에 다시 물어볼까?"를 결정하고, `gcTime`은 "언제 메모리에서 완전히 지울까?"를 결정하기 때문입니다.
- **다른 값 설정의 이점:** `gcTime`을 `staleTime`보다 길게 잡으면(SWR 패턴), 데이터가 `stale`하더라도 메모리에 남아있어 **페이지 이동 시 이전 데이터를 즉시 보여준 뒤 백그라운드에서 조용히 최신화**할 수 있습니다. (사용자는 로딩 스피너 대신 실질적인 데이터를 먼저 보게 됨)

#### 🧐 [Q&A] 데이터가 stale 상태면 어떤 일이 벌어질까?
1. **가만히 있으면 갑자기 데이터가 바뀌나요?**
   - 아니요. 기본적으로 가만히 보고만 있으면 데이터는 바뀌지 않습니다. `stale`은 "다시 가져올 준비가 됨"을 의미할 뿐입니다.
2. **그럼 언제 실질적으로 다시 가져오나요? (리패칭 트리거)**
   - **창 포커스:** 다른 탭에 갔다 돌아올 때 (`refetchOnWindowFocus`)
   - **재마운트:** 다른 페이지 갔다 돌아올 때 (`refetchOnMount`)
   - **네트워크 재연결:** 인터넷이 끊겼다 붙을 때 (`refetchOnReconnect`)
3. **가 가만히 있어도 바뀌게 하려면?**
   - `refetchInterval` 옵션을 사용하여 주식/채팅 앱처럼 주기적으로 강제 갱신할 수 있습니다.

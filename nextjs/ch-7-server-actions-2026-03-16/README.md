...
API 엔드포인트를 명시적으로 만들지 않고도, 클라이언트(form 등)에서 서버 코드를 직접 실행할 수 있게 해주는 기능
Server Actions는 **서버에서 실행되는 비동기 함수**로, 클라이언트의 폼 제출이나 이벤트에 의해 호출됩니다.

- 기존 방식 (API Routes):
  1. /api/submit 파일을 만듦
  2. 클라이언트에서 fetch('/api/submit', { method: 'POST', ... }) 호출
  3. JSON으로 변환하여 데이터 처리

- Server Actions 방식:
  1. 서버 함수("use server")를 정의
  2. <form action={myAction}>에서 해당 함수를 직접 연결
  3. Next.js가 자동으로 서버로 요청을 보내고 결과를 처리
...
Server Action임을 알리기 위해 "use server" 지시어를 사용
